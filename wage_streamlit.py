...
jp_lat_lon = jp_lat_lon.rename(columns={'pref_name': '都道府県名'}) # 日本政府のデータの列名に合わせる
#st.write(jp_lat_lon)
...
layer = pdk.Layer(
    ...
    threshold = 0.3,
    get_weight = '一人あたり賃金（相対値）' # データに複数の列があった場合にどの列データをヒートマップにするかを指定
)
layer_map = pdk.Deck(
    layers = layer,
...
max_x = df_jp_category[option_wage].max() + 50 # 横棒グラフの横（ｘ）軸の最大値がwage_listの中身に応じて変わるようにする
